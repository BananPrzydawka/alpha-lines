...
pub trait Variant {
    ...
    /// Root action returns and visit counts for supervised training.
    fn action_targets(
        _stats: &Self::Stats,
        _q: &mut [[f32; SQUARES]; 2],
        _visits: &mut [[u64; SQUARES]; 2],
    ) {}
...
impl Variant for Puct {
    ...
    fn action_targets(
        stats: &PuctStats,
        q: &mut [[f32; SQUARES]; 2],
        visits: &mut [[u64; SQUARES]; 2],
    ) {
        *visits = stats.visit;
        for player in 0..2 {
            for sq in 0..SQUARES {
                q[player][sq] = stats.q[player][sq] as f32;
            }
        }
    }
