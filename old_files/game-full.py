...
class lines_game:
    """single game instance. do not batch calls, it slows things down. faster for less than 41 games at once"""
    ...
    def get_encoded_state(self, player):
        ...
        # --- FIX: Swap piece channels for Player 1 perspective ---
        if player == 1:
...
class batched_lines_game:
    """multiple games in parralel. only use batched. faster for more than 41 games at once"""

    def __init__(self, num_games=1, device="cpu"):
        self.n = num_games
        self.device = device
        ...
        self.scores = torch.zeros(
            num_games, 2, dtype=torch.float32, device=device
        )
        self.move_counts = torch.zeros(
            num_games, dtype=torch.int32, device=device
        )
        self.finished = torch.zeros(num_games, dtype=torch.bool, device=device)
        self.half_width = width // 2

        # --- DYNAMIC DEVICE ALIGNMENT ---
        # Force all global precomputed variables to match the engine runtime device
        global _edge_mask, _nesw_col_safe, _nwse_col_safe, _nesw_valid, _nwse_valid, _nesw_score_cols_t, _nwse_score_cols_t
        _edge_mask = _edge_mask.to(device)
        _nesw_col_safe = _nesw_col_safe.to(device)
        _nwse_col_safe = _nwse_col_safe.to(device)
        _nesw_valid = _nesw_valid.to(device)
        _nwse_valid = _nwse_valid.to(device)
        _nesw_score_cols_t = _nesw_score_cols_t.to(device)
        _nwse_score_cols_t = _nwse_score_cols_t.to(device)
    ...
    def step_with_actions(self, idx_0, idx_1):
        """Advances games using explicit 1D action indices after validating move legality.
        
        Raises a ValueError if any active game receives an invalid move, preventing
        any board state updates across the entire batch.
        """
        active = ~self.finished
        if not active.any():
            return

        # 1. Generate legal move masks for both players
        playable = (self.boards == playable_square)
        masks_0 = playable.float()
        masks_1 = playable.float()
        ...
        # Flatten masks to align with 1D action indices: shape (num_games, height * width)
        flat_masks_0 = masks_0.view(self.n, -1)
        flat_masks_1 = masks_1.view(self.n, -1)

        # 2. Verify if the chosen indices are legal (mask value == 1.0)
        ...
        # Check if any active game has an invalid move
        ...
        # 3. Convert 1D indices to 2D coordinates (Safe to proceed for all active games)
        r0, c0 = idx_0 // width, idx_0 % width
        r1, c1 = idx_1 // width, idx_1 % width

        collision = (r0 == r1) & (c0 == c1)

        # Apply non-colliding moves
        nc = active & ~collision
        if nc.any():
            i = nc.nonzero(as_tuple=True)[0]
            self.boards[i, r0[i], c0[i]] = player_0_mark
    ...
    def clone_states_to_batch(self, batch_indices, target_game):
        """Copies states from specific indices of this batch into another game instance."""
        target_game.boards[batch_indices] = self.boards[batch_indices].clone()
        target_game.scores[batch_indices] = self.scores[batch_indices].clone()
        target_game.move_counts[batch_indices] = self.move_counts[batch_indices].clone()
        target_game.finished[batch_indices] = self.finished[batch_indices].clone()
...
def print_game(game, index=None, player=0):
    # 1. Retrieve the correct 3D encoding tensor (7, H, W)
    if isinstance(game, lines_game):
        encoding = game.get_encoded_state(player)
    elif isinstance(game, batched_lines_game):
        if index is None:
            raise ValueError("index must be specified for a batched_lines_game.")
        encoding = game.get_encoded_states(player)[index]
    else:
        raise TypeError("game must be an instance of lines_game or batched_lines_game.")

    # 2. Extract height and width from the encoding shape
    h, w = encoding.shape[1], encoding.shape[2]

    # 3. Retrieve and decode normalized scores back to raw integer values
    norm = w * h / 2
    if player == 0:
        s0 = int(round(encoding[5, 0, 0].item() * norm))
        s1 = int(round(encoding[6, 0, 0].item() * norm))
    else:
        s1 = int(round(encoding[5, 0, 0].item() * norm))
        s0 = int(round(encoding[6, 0, 0].item() * norm))
    
    print([s0, s1])

    # 4. Reconstruct the original board values using argmax across the channels
    # Channel indices match global constants:
    # 0: non_playable_square, 1: playable_square (modified by legal mask), 2: removed_square,
    # 3: player_0_mark, 4: player_1_mark
    # Note: Channel 1 is altered by the turn-0 mask, but channel 0/2/3/4 remain mutually exclusive
    # with the actual underlying cell states.
    board_state = torch.argmax(encoding[:5], dim=0)

    # 5. Render the board ASCII visualization
    border = "+" + "-" * (w * 2) + "+"
    print(border)
    for r in range(h):
        line = "|"
        for c in range(w):
            ...
            if v == 0:    # non_playable_square
                line += "  "
            elif v == 1:  # playable_square
                line += "--"
            elif v == 2:  # removed_square
                line += "##"
            elif v == 3:  # player_0_mark
                line += "||"
            elif v == 4:  # player_1_mark
                line += "oo"
        line += "|"
        print(line)
    print(border)